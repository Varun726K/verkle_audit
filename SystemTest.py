...
def test_system_flow():
    ...
    # In reality, KGC generates params. Here, DataOwner initializes KZG 
    # and we share that instance with CloudProvider for prototype simplicity.
    print("[1] Initializing Data Owner...")
    do = DataOwner()
    
    print("[2] Initializing Cloud Provider...")
    csp = CloudProvider()
    # Share the public parameters (SRS) from DO's KZG instance
    csp.load_kzg(do.kzg)
    
    # 2. Prepare Data (Data Owner)
    print("\n[3] Data Owner: Processing File...")
    ...
    for i, p in enumerate(proofs):
        ...
        # Using the verify function from DO's kzg instance (acting as Auditor)
        is_valid = do.kzg.verify(root, proof_pi, z, y)
